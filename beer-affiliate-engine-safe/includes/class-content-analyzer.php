...
class Beer_Affiliate_Content_Analyzer {
     ...
     * 
     * @param string $content コンテンツ
     * @return array 抽出された地名リスト
     */
    public function extract_locations($content) {
        $locations = array();
        
        // 一般的な日本の都市名
        $cities = array(
            '東京', '大阪', '京都', '札幌', '福岡', '横浜', '名古屋', '神戸', 
            '仙台', '広島', '金沢', '那覇', '長崎', '鹿児島', '新潟'
        );
        
        // 海外の都市名（ビール関連）
        $international_cities = array(
            'シアトル', 'ポートランド', 'サンディエゴ', 'ニューヨーク',
            'ミュンヘン', 'ベルリン', 'プラハ', 'ブリュッセル', 'ダブリン',
            'ロンドン', 'アムステルダム', 'コペンハーゲン'
        );
        ...
        foreach ($all_cities as $city) {
            if (mb_strpos($content, $city) !== false) {
                $locations[] = array(
                    'name' => $city,
                    'type' => in_array($city, $international_cities) ? 'international' : 'domestic'
                );
            }
        }
