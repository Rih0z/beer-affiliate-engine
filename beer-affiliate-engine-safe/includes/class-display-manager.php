...
class Beer_Affiliate_Display_Manager {
    ...
    public function render($links, $template_type = 'card') {
        ...
        switch ($template_type) {
            ...
            case 'user-friendly':
                return $this->render_user_friendly_template($links);
            case 'card':
            default:
    ...
    /**
     * ユーザーフレンドリーテンプレートでリンクをレンダリング
     * 
     * @param array $links リンク情報の配列
     * @return string HTML出力
     */
    protected function render_user_friendly_template($links) {
        $template_file = $this->template_dir . 'user-friendly.php';
